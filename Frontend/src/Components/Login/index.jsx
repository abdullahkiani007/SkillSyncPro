...
const Login = ({ role }) => {
  ...
  const [path, setPath] = useState(route)
  const [loading, setLoading] = useState(false)
  const [submitted, setSubmitted] = useState(false) // Track whether form is submitted
  const navigate = useNavigate()
  const dispatch = useDispatch()
        ...
        >
          ...
          <div className='flex flex-col items-center justify-center'>
            <input
              ...
              placeholder='Email'
            />
            {submitted && errors.email && touched.email && (
              <p className='text-red-700'>{errors.email}</p>
            )}
            ...
            <input
              ...
              onChange={handleInputChange}
            />
            {submitted && errors.password && touched.password && (
              <p className='text-red-700'>{errors.password}</p>
            )}
            ...
            <button
              className='rounded-2xl m-2 text-white bg-primary w-fit px-4 py-2 shadow-md hover:text-primary hover:bg-white transition duration-200 ease-in disabled:bg-primary disabled:opacity-70 disabled:text-white'
              onClick={handleClick}
              disabled={false} // Disabled logic removed
            >
              Sign In
            </button>
          </div>
