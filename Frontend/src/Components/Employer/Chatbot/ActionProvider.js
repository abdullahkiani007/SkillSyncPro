class ActionProvider {
    ...
    handleScheduleInterview = () => {
      const message = this.createChatBotMessage("To schedule an interview, please select a date and time.");
      this.setState((prev) => ({ ...prev, messages: [...prev.messages, message] }));
    ...
    handleApplicantMetrics = async () => {
      ...
      this.setState((prev) => ({
        ...
        messages: [
          ...prev.messages,
          this.createChatBotMessage(`Applicant metrics: ${JSON.stringify(metrics)}`),
        ],
      }));
    };
