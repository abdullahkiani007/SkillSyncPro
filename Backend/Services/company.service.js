...
const companyService = {
  async register(company) {
    ...
    try {
      ...
      if (employer.company) {
        console.log(
          `Employer with id: ${company.createdBy} already has a company`
        );
        return {
          status: 400,
      ...
      // Associate the new company with the employer
  ...
  async leaveCompany(userId) {
    try {
      ...
      if (
        !company.unAuthEmployees.includes(employer._id) &&
        !company.employees.includes(employer._id)
      ) {
      ...
      company.unAuthEmployees = company.unAuthEmployees.filter(
  ...
  async getEmployees(companyId) {
    try {
      ...
      const authEmployees = await Promise.all(
      ...
      const unAuthEmployees = await Promise.all(
        company.unAuthEmployees.map(async (employee) => {
          employee = await employee.populate("user");
          return new EmployerDTO(employee);
        })
      );
